...
X-Ray debugs multi-step, non-deterministic pipelines (LLM workflows, filtering systems, ranking algorithms). Unlike traditional tracing that tracks *performance*, X-Ray tracks *decision reasoning*.
...
│ ┌────────────────────────────────────────────────────────┐  │
│ │ async function findCompetitor(product) {               │  │
│ │   const xray = new XRaySDK();                          │  │
│ │   const run = xray.startRun('competitor_selection');   │  │
│ │                                                         │  │
│ │   const step1 = run.addStep('generate_keywords');      │  │
...
│ (Express + Node.js)                                          │
│ ┌────────────────────────────────────────────────────────┐  │
│ │ POST /api/runs         ← Ingest trace data             │  │
│ │ GET  /api/runs/:id     ← Get specific run              │  │
│ │ GET  /api/runs         ← List all runs                 │  │
│ │ POST /api/runs/query   ← Advanced queries              │  │
│ └────────────────────────────────────────────────────────┘  │
│                              │                               │
│                              │ SQL INSERT/SELECT             │
│                              ▼                               │
...
│ Query Interface                                              │
│                                                              │
│ Developer queries for debugging:                            │
│  - GET /api/runs/{id}        - Debug specific run           │
│  - POST /api/runs/query      - Find patterns                │
│  - Dashboard (future)        - Real-time visualization      │
└─────────────────────────────────────────────────────────────┘
```
...
Developer Code      SDK                 API             Database
      │              │                   │                  │
      │ startRun()   │                   │                  │
      ├─────────────▶│                   │                  │
      │              │ Create Run obj    │                  │
...
└────────────────────────────────────────┘
        │
        ├─▶ Accepted (30 candidates)
        │   └─▶ Keep ALL (keepAllOutputs = true)
        │       ✅ Store: 30 candidates
        │
        ├─▶ Near Threshold (20 candidates)
        │   └─▶ Keep ALL (edge cases for debugging)
        │       ✅ Store: 20 candidates
        │
        └─▶ Clearly Rejected (4,950 candidates)
            └─▶ Sample 1% (sampleRate = 0.01)
                ✅ Store: 50 candidates

Total Stored: 100 candidates (98% reduction)
Original: 5,000 candidates
```
...
```
┌─────────────────────┐ ┌─────────────────────┐ ┌─────────────────────┐
│ competitor_selection│ │ categorization      │ │ fraud_detection     │
│                     │ │                     │ │                     │
│ Step 1: keywords    │ │ Step 1: extract     │ │ Step 1: fetch_user  │
│ Step 2: search      │ │ Step 2: match       │ │ Step 2: velocity    │
│ Step 3: filter_price│ │ Step 3: filter_conf │ │ Step 3: filter_risk │
│         ├─ 5000→450 │ │         ├─ 1000→100 │ │         ├─ 500→50   │
│         └─ elim: 91%│ │         └─ elim: 90%│ │         └─ elim: 90%│
│                  ✅ │ │                  ✅ │ │                  ✅ │
│ Step 4: llm_check   │ │ Step 4: llm_verify  │ │ Step 4: llm_score   │
│         ├─ 120→4    │ │         ├─ 100→10   │ │         ├─ 50→5     │
│         └─ elim: 97%│ │         └─ elim: 90%│ │         └─ elim: 90%│
│                  ⚠️ │ │                  ✅ │ │                  ✅ │
└─────────────────────┘ └─────────────────────┘ └─────────────────────┘
        │                       │                       │
        └───────────────────────┼───────────────────────┘
...
```json
{
  "steps": [
    ...
    {"step_name": "filter_price", "candidates_in": 5000, "candidates_out": 450, "elimination_rate": 0.91},
    {"step_name": "llm_relevance_check", "candidates_in": 120, "candidates_out": 4, "elimination_rate": 0.97,
      "candidates": [
        {"data": {"name": "Laptop Stand"}, "status": "accepted"},
        {"data": {"name": "Samsung Charger"}, "status": "rejected", "reason": "Price too low"}
      ]
    }
  ]
}
...
step2.recordFiltering(products, filtered, 'price_range', 'range', {min:100, max:500});
